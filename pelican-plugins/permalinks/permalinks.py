...
class PermalinkGenerator(Generator):
    ...
    def generate_context(self):
        ...
        Setup context
        """
        self.permalink_output_path = os.path.join(self.output_path, self.settings["PERMALINK_PATH"])
    ...
    def generate_output(self, writer=None):
        ...
        for content in itertools.chain(self.context["articles"], self.context["pages"]):
            for permalink_id in content.get_permalink_ids_iter():
                permalink_path = os.path.join(self.permalink_output_path, permalink_id) + ".html"

                redirect_string = REDIRECT_STRING.format(url=article_url(content), title=content.title)
...
def get_permalink_path(self):
    ...
    return "/{settings[PERMALINK_PATH]}/{first_permalink}.html".format(settings=self.settings, first_permalink=first_permalink_id)
