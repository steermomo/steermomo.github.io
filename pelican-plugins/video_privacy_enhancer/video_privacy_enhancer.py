...
def check_for_thumbnail_directory(pelican_output_path):
    ...
    try:
        ...
        logger.error(
            "Video Privacy Enhancer Plugin: Error in checking if thumbnail folder exists and making the directory if it doesn't: %s", e
...
def download_thumbnail(video_id_from_shortcode, video_thumbnail_url, video_service_name, pelican_output_path):
    ...
    if not os.path.exists(
        pelican_output_path + "/" + output_directory_for_thumbnails + "/" + video_service_name + "_" + video_id_from_shortcode + ".jpg"
        ...
        six.moves.urllib.request.urlretrieve(
            video_thumbnail_url, pelican_output_path + "/" + output_directory_for_thumbnails + "/" + video_service_name + "_" + video_id_from_shortcode + ".jpg"
...
def process_shortcodes(data_passed_from_pelican):
    ...
    for video_service_name, video_service_information in six.iteritems(dictionary_of_services):
        ...
        all_instances_of_the_shortcode = re.findall(
            "(?<!\\\)\!" + shortcode_to_search_for_not_including_exclamation_point + ".*?\)", data_passed_from_pelican._content
        ...
        if (
            ...
            for shortcode_to_parse in all_instances_of_the_shortcode:
                video_id_from_shortcode = re.findall("(?<=" + shortcode_to_search_for_not_including_exclamation_point + "\().*?(?=\))", shortcode_to_parse)[
                    0
                ...
                replace_shortcode_in_text = re.sub(
                    r"\!" + shortcode_to_search_for_not_including_exclamation_point + "\(" + video_id_from_shortcode + "\)",
