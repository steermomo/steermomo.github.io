...
class TestSummary(unittest.TestCase):
    ...
    def _copy_page_kwargs(self):
        ...
        for key in page_kwargs:
            if not isinstance(page_kwargs[key], dict):
                break
            page_kwargs[key] = dict([(subkey, page_kwargs[key][subkey]) for subkey in page_kwargs[key]])
    ...
    def test_end_summary(self):
        ...
        page_kwargs["content"] = TEST_SUMMARY + "<!-- PELICAN_END_SUMMARY -->" + TEST_CONTENT
    ...
    def test_begin_end_summary(self):
        page_kwargs = self._copy_page_kwargs()
        del page_kwargs["metadata"]["summary"]
        page_kwargs["content"] = "FOOBAR<!-- PELICAN_BEGIN_SUMMARY -->" + TEST_SUMMARY + "<!-- PELICAN_END_SUMMARY -->" + TEST_CONTENT
        page = Page(**page_kwargs)
        summary.extract_summary(page)
        # test both the summary and the marker removal
        self.assertEqual(page.summary, TEST_SUMMARY)
