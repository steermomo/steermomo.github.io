...
class _GitWrapperCommon(object):
    ...
    def __init__(self, repo_path):
        self.git = Git()
        self.git.update_environment(GIT_CONFIG_NOSYSTEM="true", HOME=os.getcwd(), XDG_CONFIG_HOME=os.getcwd())
    ...
    def is_file_managed_by_git(self, path):
        ...
        """
        status, _stdout, _stderr = self.git.execute(["git", "ls-files", path, "--error-unmatch"], with_extended_output=True, with_exceptions=False)
        return status == 0
    ...
    def is_file_modified(self, path):
        ...
        """
        status, _stdout, _stderr = self.git.execute(["git", "diff", "--quiet", "HEAD", path], with_extended_output=True, with_exceptions=False)
        return status != 0
