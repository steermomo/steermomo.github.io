...
class VocabularyGenerator(CachingGenerator):
    ...
    def generate_vocabulary_context(self, vocabulary_file_name, path_to_vocabulary):
        logger.debug("Generating__ vocabulary context for " + path_to_vocabulary + "/" + vocabulary_file_name)
        ...
        if voc is None:
            try:
                ...
                logger.error("Could not process %s\n%s", vocabulary_file_name, e, exc_info=self.settings.get("DEBUG", False))
...
class RdfReader(BaseReader):
    ...
    def read(self, source_path):
        ...
        meta = {}
        queries = [f for f in listdir(self.settings["VOC_QUERIES_PATH"]) if (isfile(join(self.settings["VOC_QUERIES_PATH"], f)) and f.endswith(".sparql"))]
