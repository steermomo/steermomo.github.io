...
前者主要是Bagging和随机森林, 后者的代表是Boosting方法.



Bagging是对数据做行抽样, 使用bootstrap sampling采样m次, 可并行获得m个模型, 再将m个模型进行组合. 每个模型的权重相同.

随机森林是对数据做列抽样,  在训练决策树时随机选择属性.



Boosting是改变数据的分布, 有AdaBoost和Gradient Boosting.  AdaBoost方法是多轮训练, 在每一轮中,提升前一轮被错误分类的样本的权值, 具体的内容这里不提.

Gradient Boosting是直接改变弱分类器的学习目标, 学习当前组合模型的梯度值. 具体的算法内容后面会讲到.
...
梯度下降没啥说的,  根据参数空间当前的梯度值, 可以迭代地对参数进行优化.
...
上面的优化过程中, 固定了基函数的形式, 以及参数的数量, 优化过程是在参数空间完成的. 如果基函数是非参数化的, 就引入了函数空间的优化.
...
这里导数为
$$
g_m(x)=\left[\frac{\partial\phi(F(x))}{\partial F(x)}\right]_{F(x)=F_{m-1}(x)}=\left[\frac{\partial E_y[L(y, F(X))|x]}{\partial F(x)}\right]_{F(x)=F_{m-1}(x)}
$$
...
上面的参数空间优化推导, 是在考虑无限数据点的情况下得到的, 然而实际上训练集中的数据是有限的, 因此不能准确地估计公式中的期望.  



那就从最开始定义的形式, 将其写成参数化的基函数, 这里$a_m=\{a_1, a_2, \dots\}$, 是个无穷集合, 最优参数为
$$
...
求出当前基函数(基学习器)的形式后, 同样是加法模型, 有
$$
F_m(x) = F_{m-1}(x)+\beta_m h(x;a_m) \tag{GBM 10}
$$


但是有时候，对于特定的损失函数或是基学习器，(GBM 9) 式很难求解.



这时候需要换一个角度去看之前设置的(GBM 10)式,  类比于梯度下降法, (GBM 10)式可以看作是在当前位置$F_{m-1}(x)$沿着方向$h(x;a_m)$前进了$\beta_m$.



如果类似无约束下的梯度, 则有
$$
-g_m(x)=-\left[\frac{\partial L(y_i, F(x_i))}{\partial F(x)}\right]_{F(x)=F_{m-1}(x)}
$$


这里是在N个数据点的情况下， 获得的$-g_m(x_i)=\{-g_m(x_i)\}_1^N$.  这是在有限个数据点下获得的梯度, 并不能直接推广到无限数据点的函数空间中, 但是我们可以选择基函数的形式, 使得$h_m=\{h(x_i;a_m\}_1^N$, 尽可能平行于$-g_m$
...
因此， 并不是直接求解在平滑约束下的(GBM 9)式获取最优参数,  通过让$h(x;a)$拟合$\{\hat{y}_i=-g_m(x_i)\}_{i=1}^N$, 将约束作用到无约束下的解上. 这样就将比较难求解的最小化问题(GBM 9)式转换为最小二乘(GBM 11),  随后的(GBM 12)式是单参数的优化求解.
...
在XGBoost中, 对每个特征, 会学到一个最优的划分方向, 在该特征上出现缺失值时, 就把样本划分到学到的最优方向上.
...
CatBoost 的基本原理是解决原始 GBDT 中的各种数据偏移问题。
